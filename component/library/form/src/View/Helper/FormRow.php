...
class FormRow extends \Zend\Form\View\Helper\FormRow
{
    ...
    public function render(\Zend\Form\ElementInterface $oElement,
        ...
        if ($oElement->getAttribute('required')) {
            ...
            if (empty($aLabelAttributes['class'])) {
                $aLabelAttributes['class'] = 'required';
            } elseif (strpos($aLabelAttributes['class'], 'required')
                === false
            ) {
                $aLabelAttributes['class'] .= ' required';
            }
    ...
    protected function renderElement(\Zend\Form\ElementInterface $oElement)
    {
        ...
        if ($sLabelContent = $this->renderLabel($oElement)) {
            ...
            if ($sElementType === 'checkbox') {
                if (!$oElement->getLabelAttributes()
                    && $this->labelAttributes
                ) {
            ...
            elseif ($oElement instanceof \Zend\Form\Element\Button) {
                ...
                switch ($sLayout) {
                    ...
                    case \TwbBundle\Form\View\Helper\TwbBundleForm::LAYOUT_INLINE:
                        if (empty($aLabelAttributes['class'])) {
                            ...
                            '/(\s|^)sr-only(\s|$)/', $aLabelAttributes['class']
                        )
                        ) {
                            $aLabelAttributes['class'] = trim(
                                $aLabelAttributes['class'] . ' sr-only'
                ...
                $sLabelOpen = $oLabelHelper->openTag(
                    $oElement->getAttribute('id') ? $oElement
                        : $aLabelAttributes
                );
        ...
        switch ($sLayout) {
            case null:
            case \TwbBundle\Form\View\Helper\TwbBundleForm::LAYOUT_INLINE:
                $sElementContent = $sLabelOpen . $sLabelContent . $sLabelClose
            ...
            case \TwbBundle\Form\View\Helper\TwbBundleForm::LAYOUT_HORIZONTAL:
                ...
                return sprintf(
                    ...
                    $sLabelOpen . $sLabelContent . $sLabelClose, $sClass,
                    $sElementContent
                );

            default:
                throw new \DomainException(
                    'Layout "' . $sLayout . '" is not valid'
                );
        }
    }
    ...
    protected function renderHelpBlock(\Zend\Form\ElementInterface $oElement)
    {
        return ($sHelpBlock = $oElement->getOption('help-block')) ? sprintf(
            self::$helpBlockFormat, $this->getEscapeHtmlHelper()->__invoke(
            ($oTranslator = $this->getTranslator()) ? $oTranslator->translate(
                $sHelpBlock, $this->getTranslatorTextDomain()
            ) : $sHelpBlock
        )
        ) : '';
    }
